...
RUN \
    mkdir pkg-config; \
    wget -qO- --no-check-certificate https://pkg-config.freedesktop.org/releases/pkg-config-$PKGCONFIG_VERSION.tar.gz \
...
RUN \
    mkdir sqlite3; \
    wget --no-check-certificate -qO- https://www.sqlite.org/2020/sqlite-autoconf-3330000.tar.gz \
...
RUN \
    mkdir proj; \
    wget --no-check-certificate -qO- http://download.osgeo.org/proj/proj-$PROJ_VERSION.tar.gz \
...
RUN \
    mkdir nghttp2; \
    wget --no-check-certificate -qO- https://github.com/nghttp2/nghttp2/releases/download/v${NGHTTP2_VERSION}/nghttp2-${NGHTTP2_VERSION}.tar.gz \
...
RUN \
    mkdir curl; \
    wget --no-check-certificate -qO- https://curl.haxx.se/download/curl-${CURL_VERSION}.tar.gz \
...
RUN \
    mkdir geos; \
    wget --no-check-certificate -qO- http://download.osgeo.org/geos/geos-$GEOS_VERSION.tar.bz2 \
...
RUN \
    mkdir szip; \
    wget --no-check-certificate -qO- https://support.hdfgroup.org/ftp/lib-external/szip/$SZIP_VERSION/src/szip-$SZIP_VERSION.tar.gz \
...
RUN \
    mkdir hdf4; \
    wget --no-check-certificate -qO- https://support.hdfgroup.org/ftp/HDF/releases/HDF$HDF4_VERSION/src/hdf-$HDF4_VERSION.tar \
...
RUN \
    mkdir hdf5; \
    wget --no-check-certificate -qO- https://support.hdfgroup.org/ftp/HDF5/releases/hdf5-${HDF5_VERSION%.*}/hdf5-${HDF5_VERSION}/src/hdf5-$HDF5_VERSION.tar.gz \
...
RUN \
    mkdir netcdf; \
    wget --no-check-certificate -qO- https://github.com/Unidata/netcdf-c/archive/v$NETCDF_VERSION.tar.gz \
...
RUN \
    mkdir webp; \
    wget --no-check-certificate -qO- https://storage.googleapis.com/downloads.webmproject.org/releases/webp/libwebp-${WEBP_VERSION}.tar.gz \
...
RUN \
    mkdir zstd; \
    wget --no-check-certificate -qO- https://github.com/facebook/zstd/archive/v${ZSTD_VERSION}.tar.gz \
...
RUN \
    mkdir openjpeg; \
    wget --no-check-certificate -qO- https://github.com/uclouvain/openjpeg/archive/v$OPENJPEG_VERSION.tar.gz \
...
RUN \
    mkdir jpeg; \
    wget --no-check-certificate -qO- https://github.com/libjpeg-turbo/libjpeg-turbo/archive/${LIBJPEG_TURBO_VERSION}.tar.gz \
...
RUN \
    mkdir geotiff; \
    wget --no-check-certificate -qO- https://download.osgeo.org/geotiff/libgeotiff/libgeotiff-$GEOTIFF_VERSION.tar.gz \
...
RUN \
    mkdir gdal; \
    wget --no-check-certificate -qO- http://download.osgeo.org/gdal/$GDAL_VERSION/gdal-$GDAL_VERSION.tar.gz \
...
RUN \
    mkdir openssl; \
    wget --no-check-certificate -qO- https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz \
